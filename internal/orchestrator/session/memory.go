...
// InMemorySessionStore implements Manager using in-memory storage
type InMemorySessionStore struct {
	sessions map[string]*orchestratorpb.Session
	mu       sync.RWMutex
}

var _ models.SessionManager = (*InMemorySessionStore)(nil)
...
func (s *InMemorySessionStore) CreateSession(ctx context.Context, sessionReq *orchestratorpb.CreateSessionRequest) (*orchestratorpb.Session, error) {
	s.mu.Lock()
	defer s.mu.Unlock()

	id := uuid.New()
	if _, exists := s.sessions[id.String()]; exists {
		return nil, fmt.Errorf("session with ID %s already exists", id.String())
	}
...
func (s *InMemorySessionStore) GetSession(ctx context.Context, sessionID string, userID string) (*orchestratorpb.Session, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *InMemorySessionStore) ListSessions(ctx context.Context, userID string, limit int32, pageToken string) ([]*orchestratorpb.Session, string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	...
	// Apply pagination
	if int(offsetInt) >= len(userSessions) {
	...
	return userSessions[offsetInt:end], "", nil
}
...
func (s *InMemorySessionStore) DeleteSession(ctx context.Context, sessionID string, userID string, force bool) error {
	s.mu.Lock()
	defer s.mu.Unlock()
